...
namespace Fahrzeugpark
{
	public abstract class Fahrzeug
	{
		...
		// Abstrakte Methode zum Hupen
		public abstract void Hupe();

        // Methode zur zufälligen Generierung von Fahrzeugen
		public static Fahrzeug GeneriereFahreug (string name)
		{
			Random random = new Random();
			int zufallsZahl = random.Next(1, 4); // Zufallszahl zwischen 1 und 3
			switch (zufallsZahl)
			{
				case 1:
					return new Flugzeug("Flugzeug " + name, random.Next(200, 2000), random.Next(50000, 1_000_000_000), random.Next(5000, 20000));
				case 2:
		...
		{
			if (this.MotorLaeuft)
				return $"{this.Name} kostet {this.Preis}Euro und fährt momentan " +
					$"mit {this.AktGeschwindigkeit} von maximal {this.MaxGeschwindigkeit}km/h.";
			else
				return $"{this.Name} kostet {this.Preis}Euro und könnte maximal " +
